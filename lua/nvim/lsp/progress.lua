...
M.lsp_progress_callback = function(ev)
  local client = vim.lsp.get_client_by_id(ev.data.client_id)
...
function M.statusline_get_lsp_progress()
...
-- vim.opt.statusline = vim.opt.statusline .. '%{%v:lua.get_lsp_progress()%}'
M.echo = function(ev)
  ...
  local icon = value.kind == 'end' and '' or Snacks.util.spinner()
