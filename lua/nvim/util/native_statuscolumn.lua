...
---@class native.statuscolumn.Config
---@field left string[] -- priority of signs on the left (high to low)
---@field right string[] -- priority of signs on the right (high to low)
---@field folds { open: boolean, git_hl: boolean }
---@field git { patterns: string[] }
---@field refresh number -- refresh cache interval in ms
...
---@alias native.statuscolumn.Sign.type "mark"|"sign"|"fold"|"git"
---@alias native.statuscolumn.Sign {name:string, text:string, texthl:string, priority:number, type:native.statuscolumn.Sign.type}

-- Cache for signs per buffer and line
---@type table<number,table<number,native.statuscolumn.Sign[]>>
local sign_cache = {}
...
function M.setup()
  ...
  vim.api.nvim_set_hl(0, 'NativeStatusColumnMark', { link = 'DiagnosticHint', default = true })
  
  -- Set up timer to clear cache
...
function M.buf_signs(buf)
  -- Get regular signs
  ---@type table<number, native.statuscolumn.Sign[]>
  local signs = {}
...
---@param buf number
---@param lnum number
---@return native.statuscolumn.Sign[]
...
---@private
---@param sign? native.statuscolumn.Sign
...
function M._get()
  ...
  if show_signs then
    ...
    if #signs > 0 then
      local signs_by_type = {} ---@type table<native.statuscolumn.Sign.type,native.statuscolumn.Sign>
      for _, s in ipairs(signs) do
        signs_by_type[s.type] = signs_by_type[s.type] or s
      end

      ---@param types native.statuscolumn.Sign.type[]
