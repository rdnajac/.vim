...
local foo = function(f) return vim.fn.fnamemodify(f, ':h:t') end
...
local M = {
  ...
  ['fnamemodify :t'] = function()
    return vim.tbl_map(function(path) return vim.fn.fnamemodify(path, ':t:r') end, files)
  end,
  ['regex match'] = function()
    return vim.tbl_map(function(path) return path:match('^.+/(.+)%.lua$') end, files)
  end,
