...
command addr=other -bang -range=0      -nargs=? -complete=expression PPmsg
      \ if !empty(<q-args>) |
      \   let v:errmsg = '' |
...
command addr=other -range=-1 -nargs=1 -complete=command Verbose
      \ ':exe scriptease#verbose_command(<count> == -1 ? "" : <count>, <q-args>)'

command addr=other -bar -count=0 Scriptnames
      \ call setqflist(scriptease#scriptnames_qflist()) |
      \ copen |
      \ <count>

command addr=other '-bar -bang -nargs=? -range=-1 Messages'
...
command addr=other '-range=-1 -nargs=? -complete=command Time'
      \ 'exe scriptease#time_command(<q-args>, <count>)'

command addr=other '-bar -bang -range=1 -nargs=1 -complete=customlist,scriptease#complete Ve'
      \ 'execute scriptease#open_command(<count>,"edit<bang>",<q-args>,0)'
command addr=other '-bar -bang -range=1 -nargs=1 -complete=customlist,scriptease#complete Vedit'
      \ 'execute scriptease#open_command(<count>,"edit<bang>",<q-args>,0)'
command addr=other '-bar -bang -range=1 -nargs=1 -complete=customlist,scriptease#complete Vopen'
      \ 'execute scriptease#open_command(<count>,"edit<bang>",<q-args>,1)'
command addr=other '-bar -bang -range=1 -nargs=1 -complete=customlist,scriptease#complete Vsplit'
      \ execute scriptease#open_command(<count>,"split",<q-args>,<bang>0)
command addr=other '-bar -bang -range=1 -nargs=1 -complete=customlist,scriptease#complete Vvsplit'
      \ execute scriptease#open_command(<count>,"vsplit",<q-args>,<bang>0)
command addr=other -bar -bang -range=1 -nargs=1 -complete=customlist,scriptease#complete Vtabedit
      \ execute scriptease#open_command(<count>,"tabedit",<q-args>,<bang>0)
command addr=other -bar -bang -range=1 -nargs=1 -complete=customlist,scriptease#complete Vpedit
      \ execute scriptease#open_command(<count>,"pedit<bang>",<q-args>,0)

command addr=other -bar -bang -range=1 -nargs=1 -complete=customlist,scriptease#complete Vread
